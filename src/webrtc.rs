...
impl WebRTCService {
    ...
    pub async fn set_remote_description(
        ...
        let remote_description = match sdp_type {
            RTCSdpType::Offer => RTCSessionDescription::offer(sdp)
                .map_err(|e| Error::new(format!("Failed to create RTCSessionDescription (Offer): {:?}", e)))?,
            RTCSdpType::Answer => RTCSessionDescription::answer(sdp)
                .map_err(|e| Error::new(format!("Failed to create RTCSessionDescription (Answer): {:?}", e)))?,
            RTCSdpType::Pranswer => RTCSessionDescription::pranswer(sdp)
                .map_err(|e| Error::new(format!("Failed to create RTCSessionDescription (PrAnswer): {:?}", e)))?,
   ...
   pub async fn create_data_channel(&self, label: &str) -> Result<Arc<RTCDataChannel>, Error> {
    ...
    self.peer_connection.create_data_channel(label, Some(data_channel_init)).await
}
